...
router.post('/account', async (req, res) => {
  ...
  try {
    ...
    if (action === 'profile') {
      ...
      await pool.query('UPDATE customers SET name = $1, email = $2, address = $3 WHERE id = $4', [name, email, address || '', req.session.customerId]);
      req.session.customerName = name;
      req.session.customerEmail = email;
    ...
    if (action === 'password') {
      ...
      await pool.query('UPDATE customers SET password = $1 WHERE id = $2', [hash, req.session.customerId]);
